...
async def edit_menu_desc(message: types.Message, state: FSMContext):
    ...
    await message.answer("Введите новое описание для меню:")
...
async def save_menu_desc(message: types.Message, state: FSMContext):
    ...
    await message.answer("Теперь отправьте новые медиафайлы или напишите 'Готово'")
...
async def clear_menu_media(message: types.Message):
    ...
    if data["menu_items"]:
        ...
        save_json(JSON_PATH, data)
        await message.answer("🗑 Все медиафайлы из меню удалены")
    else:
